...
// Set up CORS to allow the frontend to connect to the backend
const allowedFrontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';  // Use .env variable for frontend URL
...
const MONGO_URI = process.env.MONGO_URI;  // MongoDB connection URI from .env
  ...
  try {
    ...
    app.post('/upload/:roomId', upload.single('file'), async (req, res) => {
      try {
        ...
        console.log(`Uploading to: ${fileUrl}`); // Correct log placement
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('toggle_editability', async ({ roomId, userId }) => {
        const room = await roomsCollection.findOne({ roomId });
        if (!room) {
          return;
        }
      ...
      socket.on('send_text', async ({ roomId, text }) => {
        const room = await roomsCollection.findOne({ roomId });
        if (!room) return;
    ...
    // Scheduled Task to Delete Inactive Rooms After 48 Hours
